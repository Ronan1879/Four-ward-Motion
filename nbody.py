...
class particles:
    def __init__(self,m=1.0,npar=5,thres=0.01,G=1.0,dt=0.1,num_enc=np.zeros(self.pars['npars'])):
        ...
        self.pars['G']=G
        self.pars['dt']=dt
        self.num_enc=np.zeros((self.pars['npars'], self.pars['npars'])) # array keeping track of all the cloe encounters of the particles
        self.close_enc=np.zeros(self.pars['npars'])
